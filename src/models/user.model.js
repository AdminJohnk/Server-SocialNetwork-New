...
class UserClass {
  ...
  static async getSavedQuestions({ user }) {
    ...
    let result = await UserModel.findById(user)
      .select({ favorite_questions: 1 })
      .populate({
        path: 'favorite_questions',
        populate: [
          {
            path: 'user',
    ...
    result.favorite_questions = result.favorite_questions.map((q) => {
      q.answer_number = q.answers.length;
      delete q.answers;
      return q;
    });

    return result.favorite_questions.reverse();
  }
